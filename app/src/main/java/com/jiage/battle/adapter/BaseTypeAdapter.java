...
public abstract class BaseTypeAdapter<T> extends SDAdapter<T> implements SDDialogRefresh {
    private Toast mToast;
    private Handler mHandler = new Handler(Looper.getMainLooper());
    private boolean isDialog = true;
    private List<LoadingDialog> dialogLog = new ArrayList<>();

    public BaseTypeAdapter(List<T> listModel, Activity activity) {
    ...
    protected View onGetView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            int layoutId = getLayoutId(position,getItem(position), convertView, parent);
    ...
    public abstract int getLayoutId(int position, T model, View convertView, ViewGroup parent);
     ...
     * @param msg
     */
    private void dialogTimeSetting(final String msg){
        isDialog = true;
    ...
    public void onDialogLoading(String msg) {
        dialogTimeSetting(msg);
    }

    @Override
    public void onDialogLoading() {
        dialogTimeSetting(null);
    }
