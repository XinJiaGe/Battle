...
public abstract class PullToRefreshBase<T extends View> extends AutoLinearLayout implements IPullToRefresh<T>
{
	// ===========================================================
	// Constants
	// ===========================================================
	...
	private void pullEvent()
	{
		...
		if (newScrollValue != 0 && !isRefreshing())
		{
			...
			}else if(mState == State.RESET && itemDimension < Math.abs(newScrollValue)){
				setState(State.RELEASE_TO_REFRESH);
			}
		}
	}
	...
	public static enum Mode
	{
		 ...
		 * scrolling direction.
		 * 只允许用户从拉的刷新视图开始刷新。开始是上或左，取决于滚动方向。
		 */
		PULL_FROM_START(0x1),
		 ...
		 * scrolling direction.
		 * 只允许用户从拉的刷新查看最终刷新。开始是底部或右侧，取决于滚动方向。
		 */
		PULL_FROM_END(0x2),
